...
export async function hybridSearch(
  ...
  if (!opts?.queryEmbedFn && !isAvailable('embedding', providerProbe) && !willTryMultimodal) {
    ...
    let noEmbedPool = noEmbedOperational;
    let noEmbedRelSlot: RelationalEvidenceSlotDecision | undefined;
    if (relationalList.length > 0) {
      const r = ensureRelationalEvidenceSlot(noEmbedOperational, relationalList, limit, offset, {
  ...
  // fails. The upstream salvage fan-out otherwise leaves vectorArms empty
  // and drops into keyword-only despite holding valid image candidates.
  if (vectorArms.length === 0 && effectiveModality === 'both' && imageVectorList !== null) {
    pushVectorList(vectorArms, imageVectorList, 'image');
  }
  ...
  stampEvidence(operationalPool, { cosineFloor: resolvedMode.evidence_cosine_floor });
  ...
  if (adaptiveCfg.enabled && offset === 0) {
    ...
    // ('concept' -> otherMax, the breadth cap -- identical cap to the retired
    // ActVox 'concept'->'general' remap, but the decision record now reports
    // the true intent). The cache key folds this SAME intent class (ari= in
    // knobsHash v=27) so cross-intent rows never serve.
    // ActVox: the trimmed pool MUST stay `operationalPool` (not upstream's
    // `aliasHopped`) -- injectCanonicalOperationalResults runs after the alias
    // hop, so trimming aliasHopped here would silently drop every canonical
    // operational-memory result whenever adaptive return is on at offset 0.
